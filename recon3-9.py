#!/usr/bin/env python3
"""
MR.ROOT Scanner v3.9 — NetHunter Edition | REDHUNT-16-BETA | ADB-PENTEST
...
Zmiany v3.9 — SNIFF-UPDATE:
  [26] Nowy moduł sniff_scan(iface, duration, bpf) — komenda: sniff [iface] [sec] [bpf]
       • Backend scapy (preferowany) lub tshark (fallback)
       • Live protokół breakdown: TCP/UDP/DNS/HTTP/ARP/ICMP/TLS/MDNS/DHCP
       • Auto-zapis .pcap do katalogu raportów (gotowe do Wiresharka)
       • Opcjonalny filtr BPF (np. "port 80 or port 443")
       • Auto-wykrycie interfejsu jeśli nie podano
  [27] Ładowanie config.yaml — zewnętrzna konfiguracja ścieżek i parametrów
       (FRIDA_SERVER_LOCAL, JITTER_MIN/MAX, timeouty, itd.) bez edycji kodu
  [28] Batch getprop w _adb_device_info() — jedno wywołanie ADB zamiast 15
       (dramatycznie szybszy audit na wolnym USB)
  [29] Flaga --quiet — tylko wyniki, bez kolorów ANSI i ozdobników
  [30] Fix: nazwa pliku w ostrzeżeniu roota teraz dynamiczna (sys.argv[0])
...
QUIET        = False   # [v3.9] --quiet: brak ANSI, tylko wyniki
...
def _detect_base_dir():
    for candidate in [os.path.expanduser("~/MR.ROOT"), "/sdcard/MR.ROOT", "/tmp/MR.ROOT", "/MR.ROOT"]:
        try:
...
# =========================
# [v3.9] CONFIG.YAML LOADER
# =========================
def _load_config(config_path=None):
    """
    Ładuje config.yaml (opcjonalne). Nadpisuje globalne stałe.
    Szuka: ./config.yaml → BASE_DIR/config.yaml → /MR.ROOT/config.yaml
    Jeśli pyyaml nie jest zainstalowany lub plik nie istnieje — używa defaults.
    """
    global FRIDA_SERVER_LOCAL, TCPDUMP_LOCAL, TRAFFIC_DUMP_DURATION
    global JITTER_MIN, JITTER_MAX, COOLDOWN_TIME, SCAN_TIMEOUT
    global REPORT_DIR, HTML_DIR

    candidates = [
        config_path,
        "config.yaml",
        os.path.join(BASE_DIR, "config.yaml"),
        os.path.expanduser("~/MR.ROOT/config.yaml"),
    ]
    cfg_file = next((p for p in candidates if p and os.path.exists(p)), None)
    if not cfg_file:
        return  # brak pliku — defaults zostaną

    try:
        import yaml
    except ImportError:
        log("warn", "pyyaml niedostępny — config.yaml pominięty. Zainstaluj: pip install pyyaml")
        return
    ...
    try:
        ...
        log("warn", f"Błąd odczytu config.yaml: {e}")
        return

    paths    = cfg.get("paths", {})
    stealth  = cfg.get("stealth", {})
    adb_cfg  = cfg.get("adb", {})
    sniffer  = cfg.get("sniffer", {})

    if paths.get("frida_server"):   FRIDA_SERVER_LOCAL      = os.path.expanduser(paths["frida_server"])
    if paths.get("tcpdump"):        TCPDUMP_LOCAL           = os.path.expanduser(paths["tcpdump"])
    if paths.get("reports_dir"):
        REPORT_DIR = os.path.expanduser(paths["reports_dir"])
        HTML_DIR   = os.path.join(REPORT_DIR, "html")
        os.makedirs(REPORT_DIR, exist_ok=True)
        os.makedirs(HTML_DIR,   exist_ok=True)

    if stealth.get("jitter_min") is not None:   JITTER_MIN    = float(stealth["jitter_min"])
    if stealth.get("jitter_max") is not None:   JITTER_MAX    = float(stealth["jitter_max"])
    if stealth.get("rate_limit_cooldown"):       COOLDOWN_TIME = int(stealth["rate_limit_cooldown"])

    if adb_cfg.get("default_dump_seconds"):     TRAFFIC_DUMP_DURATION = int(adb_cfg["default_dump_seconds"])

    log("ok", f"[v3.9] Załadowano config.yaml: {cfg_file}")
...
FRIDA_SERVER_LOCAL  = os.path.expanduser("~/MR.ROOT/frida-server")
...
TCPDUMP_LOCAL  = os.path.expanduser("~/MR.ROOT/tcpdump")
...
{C.GREEN}{C.BOLD}
  ...
  ██║ ╚═╝ ██║██║  ██║    ██║  ██║╚██████╔╝╚██████╔╝   ██║
  ╚═╝     ╚═╝╚═╝  ╚═╝    ╚═╝  ╚═╝ ╚═════╝  ╚═════╝    ╚═╝{C.RESET}
{C.DIM}        Mobilny Skaner Sieci | by MR.ROOT | NetHunter Edition v3.9 REDHUNT-16-BETA | ADB-PENTEST{C.RESET}
"""
...
{c(C.CYAN+C.BOLD, "═══ KOMENDY INTERAKTYWNE ═══")}
  ...
  {c(C.GREEN+C.BOLD, "sniff [iface] [sec] [bpf]")} {c(C.GREEN+C.BOLD, "[v3.9] Pasywny sniffer — scapy/tshark → .pcap")}
...
# =========================
def log(level, msg):
    if QUIET and level in ("info", "scan", "cache"):
        return  # [v3.9] --quiet: pomiń info/scan/cache, pokaż tylko ok/warn/err/vuln
    icons = {
        "info":  c(C.CYAN,          "[*]"),
        "ok":    c(C.GREEN,         "[+]"),
        "warn":  c(C.YELLOW,        "[!]"),
    ...
    if QUIET:
        # [v3.9] bez kolorów ANSI w trybie quiet
        clean_msg = re.sub(r'\033\[[0-9;]*m', '', msg)
        print(f"{level.upper()}: {clean_msg}")
    else:
...
def sql_scan(url):
    ...
    report_data = {
        ...
        "scanner_version":  "MR.ROOT v3.9 REDHUNT-16-BETA | ADB-PENTEST"
    }
...
# =========================
# [v3.9] PASSIVE PACKET SNIFFER (sniff_scan)
# =========================
_SNIFF_PROTO_STATS: dict = {}

def sniff_scan(iface=None, duration=60, bpf_filter=""):
    """
    [v3.9] Pasywny packet sniffer — komenda: sniff [iface] [sec] [bpf]

    Backend: scapy (preferowany) → tshark (fallback jeśli scapy brak).
    Przechwytuje ruch pasywnie, liczy protokoły, zapisuje .pcap do REPORT_DIR.

    Parametry:
      iface      — interfejs (None = auto-wykrycie)
      duration   — czas przechwytywania w sekundach (domyślnie 60)
      bpf_filter — opcjonalny filtr BPF (np. "port 443" lub "host 192.168.1.1")

    Wymagania (jedno z poniższych):
      pip install scapy>=2.5.0 --break-system-packages
      apt install tshark  (fallback)
    """
    log("scan", f"Passive Sniffer v3.9 | iface={iface or 'auto'} | {duration}s"
        + (f" | BPF: {bpf_filter}" if bpf_filter else ""))

    ts          = now()
    safe_iface  = (iface or "any").replace("/", "_")
    pcap_path   = f"{REPORT_DIR}/sniff_{safe_iface}_{ts}.pcap"
    json_path   = f"{REPORT_DIR}/sniff_{safe_iface}_{ts}.json"
    ...
    if not _use_scapy:
        if shutil.which("tshark"):
            _use_tshark = True
            log("ok", "Backend: tshark (scapy niedostępny)")
        else:
            log("err", "Brak scapy i tshark — zainstaluj jeden z nich:")
            log("info", "  pip install scapy --break-system-packages")
            log("info", "  apt install tshark")
            return {"success": False, "error": "no_backend"}

    # ── Auto-wykrycie interfejsu ──────────────────────────────────
    if not iface:
        detected_iface, _, _, _ = detect_interface()
        iface = detected_iface or "any"
        log("info", f"Auto-wykryty interfejs: {c(C.GREEN+C.BOLD, iface)}")
    ...
    div = c(C.GREEN+C.BOLD, "═" * 60)
    print(f"\n{div}")
    print(c(C.GREEN+C.BOLD, f"  📡 PASSIVE SNIFFER — MR.ROOT v3.9"))
    print(c(C.DIM, f"  Interfejs: {iface} | Czas: {duration}s | pcap: {pcap_path}"))
    print(c(C.DIM,  "  Ctrl+C aby zatrzymać wcześniej"))
    print(f"{div}\n")
    ...
    # ══════════════════════════════════════════════════════════════
    if _use_scapy:
        from scapy.all import sniff as scapy_sniff, wrpcap, IP, IPv6, TCP, UDP
        from scapy.all import ICMP, ARP, DNS, DHCP, Raw
        ...
        def _classify(pkt):
            """Klasyfikuje pakiet i aktualizuje stats."""
            stats["total"] += 1
            proto = "Other"
            ...
            if pkt.haslayer(IP):
                ...
                stats["top_dst"][dst] = stats["top_dst"].get(dst, 0) + 1
            ...
            # Protokół transportowy / aplikacyjny
            if pkt.haslayer(ARP):
                proto = "ARP"
                ...
                # mDNS = port 5353
                sport = pkt[UDP].sport if pkt.haslayer(UDP) else 0
                dport = pkt[UDP].dport if pkt.haslayer(UDP) else 0
                proto = "mDNS" if (sport == 5353 or dport == 5353) else "DNS"
            elif pkt.haslayer(DHCP):
                proto = "DHCP"
            elif _tls_available and pkt.haslayer(TLS):
                proto = "TLS"
            ...
            # Live stats co 50 pakietów
            if stats["total"] % 50 == 0 and not QUIET:
                top = sorted(
                    [(k, v) for k, v in stats.items() if k in
                     ("TCP","UDP","DNS","HTTP","TLS","mDNS","ARP","ICMP","DHCP","Other")
                     and v > 0],
                    key=lambda x: -x[1]
                )[:5]
                proto_str = " | ".join(f"{k}:{v}" for k, v in top)
                print(c(C.DIM,
                    f"\r  📦 {stats['total']} pkt  [{proto_str}]"), end="", flush=True)

        def _callback(pkt):
            packets_captured.append(pkt)
            _classify(pkt)

        log("info", f"Przechwytywanie (scapy) przez {duration}s... Ctrl+C = stop wcześniej")
        try:
            scapy_sniff(
            ...
            log("warn", "Sniffer zatrzymany przez użytkownika.")
        except Exception as e:
            log("err", f"Błąd scapy: {e}")

        # Zapisz pcap
        if packets_captured:
            try:
                wrpcap(pcap_path, packets_captured)
                size_kb = os.path.getsize(pcap_path) // 1024
                log("ok", f"Pcap zapisany: {pcap_path} ({size_kb} KB, {len(packets_captured)} pkt)")
            except Exception as e:
                log("err", f"Błąd zapisu pcap: {e}")
        else:
            log("warn", "Nie przechwycono żadnych pakietów.")

    # ══════════════════════════════════════════════════════════════
    # BACKEND: TSHARK (fallback)
    # ══════════════════════════════════════════════════════════════
    elif _use_tshark:
        tshark_cmd = [
            "tshark",
        ...
        if bpf_filter:
            tshark_cmd += ["-f", bpf_filter]

        log("info", f"Przechwytywanie (tshark) przez {duration}s...")
        try:
            proc = subprocess.Popen(tshark_cmd, stderr=subprocess.DEVNULL)
            proc.wait(timeout=duration + 5)
        except (subprocess.TimeoutExpired, KeyboardInterrupt):
            proc.terminate()
            log("warn", "Tshark zatrzymany.")
        except Exception as e:
            log("err", f"Błąd tshark: {e}")

        if os.path.exists(pcap_path):
            size_kb = os.path.getsize(pcap_path) // 1024
            log("ok", f"Pcap zapisany: {pcap_path} ({size_kb} KB)")
        else:
            log("warn", "Tshark nie zapisał pliku pcap.")

        # Analiza pcap przez tshark -z
        log("info", "Analiza statystyk pcap przez tshark...")
        try:
            out = subprocess.check_output(
                ["tshark", "-r", pcap_path, "-q", "-z", "io,phs"],
                stderr=subprocess.DEVNULL, timeout=30
            )
            proto_lines = out.decode("utf-8", errors="ignore")
            for line in proto_lines.splitlines():
                parts = line.split()
                if len(parts) >= 2 and parts[0].lower() in \
                        ("tcp","udp","dns","http","tls","mdns","arp","icmp","dhcp"):
                    proto_name = parts[0].upper()
                    try:
                        stats[proto_name] = int(parts[1])
                        stats["total"] += int(parts[1])
                    except (ValueError, IndexError):
                        pass
        except Exception:
            pass  # statystyki nieobowiązkowe

    # ── Podsumowanie ──────────────────────────────────────────────
    print(f"\n{div}")
    print(c(C.GREEN+C.BOLD, f"  📊 SNIFF SUMMARY — {stats['total']} pakietów"))
    print(div)
    ...
    proto_items.sort(key=lambda x: -x[1])

    for proto_name, count in proto_items:
        bar_len = min(30, count * 30 // max(stats["total"], 1))
        bar     = "█" * bar_len
        pct     = count * 100 // max(stats["total"], 1)
        color   = {
            "HTTP": C.YELLOW, "TLS": C.GREEN, "DNS": C.CYAN,
            "ARP":  C.BLUE,   "TCP": C.WHITE, "UDP": C.WHITE,
        }.get(proto_name, C.DIM)
        print(f"  {c(color, proto_name):<8} {c(C.DIM, bar):<32} {count:>5} ({pct}%)")

    # Top IP sources
    if stats["top_src"]:
        top_src = sorted(stats["top_src"].items(), key=lambda x: -x[1])[:5]
        print(c(C.CYAN+C.BOLD, "\n  Top 5 źródłowych IP:"))
        for ip_addr, cnt in top_src:
            print(f"    {c(C.WHITE, ip_addr):<40} {cnt} pkt")

    # Top ports
    if stats["top_ports"]:
        top_ports = sorted(stats["top_ports"].items(), key=lambda x: -x[1])[:5]
        print(c(C.CYAN+C.BOLD, "\n  Top 5 docelowych portów:"))
        for port_num, cnt in top_ports:
            print(f"    {c(C.YELLOW, str(port_num)):<10} {cnt} pkt")

    print(f"\n{div}")
    log("info", f"Otwórz w Wireshark: wireshark \"{pcap_path}\"")

    # Zapis raportu JSON
    report_data = {
        "scanner":    "MR.ROOT v3.9 sniff_scan",
        "timestamp":  ts,
        "iface":      iface,
        "duration_s": duration,
        "bpf_filter": bpf_filter or None,
        "backend":    "scapy" if _use_scapy else "tshark",
        "pcap":       pcap_path,
        "stats":      {k: v for k, v in stats.items()
                       if k not in ("top_src", "top_dst", "top_ports")},
        "top_src":    dict(sorted(stats["top_src"].items(),
                                  key=lambda x: -x[1])[:10]),
        "top_dst":    dict(sorted(stats["top_dst"].items(),
                                  key=lambda x: -x[1])[:10]),
        "top_ports":  dict(sorted(stats["top_ports"].items(),
                                  key=lambda x: -x[1])[:10]),
    }
    try:
        with open(json_path, "w", encoding="utf-8") as f:
...
def _adb_device_info(serial):
    """
    [v3.8/v3.9] Pobiera szczegóły urządzenia przez 'adb shell getprop'.
    [v3.9] Jedno wywołanie ADB zamiast 15 — dramatycznie szybsze na wolnym USB.
    """
    PROPS = {
    ...
    info = {v: "?" for v in PROPS.values()}

    # [v3.9] Jeden getprop → parsuj wszystkie właściwości naraz
    all_props_raw, _ = _adb(serial, "shell", "getprop", timeout=20)
    for line in all_props_raw.splitlines():
        # Format: [ro.product.brand]: [Samsung]
        m = re.match(r'^\[(.+?)\]:\s*\[(.*)?\]$', line.strip())
        if m:
            prop_name, prop_val = m.group(1), m.group(2)
            if prop_name in PROPS:
                info[PROPS[prop_name]] = prop_val or "?"
...
def adb_audit(target_package=None, frida_only=False, dump_only=False,
    ...
    """
    log("scan", "ADB Pentest Audit — MR.ROOT v3.9")
    ...
    div = c(C.CYAN+C.BOLD, "═" * 60)
    print(f"\n{div}")
    print(c(C.CYAN+C.BOLD, "  📱 ADB PENTEST AUDIT — MR.ROOT v3.9"))
    print(div)
    ...
    report = {
        "serial":        serial,
        "timestamp":     now(),
        "scanner":       "MR.ROOT v3.9 ADB-PENTEST",
        "device_info":   {},
        "debug_surface": [],
        "apk_enum":      {},
        "frida":         None,
...
def parse_interactive_command(raw):
    ...
    # [v3.9] sniff [iface] [sec] [bpf_filter]
    if lower.startswith("sniff"):
        parts = raw[5:].strip().split(None, 2)
        s_iface  = parts[0] if len(parts) > 0 and not parts[0].isdigit() else None
        s_dur    = 60
        s_bpf    = ""
        idx_start = 0
        if s_iface:
            idx_start = 1
        if len(parts) > idx_start and parts[idx_start].isdigit():
            s_dur = int(parts[idx_start])
            idx_start += 1
        if len(parts) > idx_start:
            s_bpf = parts[idx_start]
        return "sniff", {"iface": s_iface, "duration": s_dur, "bpf": s_bpf}, None
...
# =========================
def main():
    global STRICT_SSL, FULL_PORTS, SQL_RESUME, QUIET

    parser = argparse.ArgumentParser(description="MR.ROOT Scanner v3.9 REDHUNT-16-BETA | ADB-PENTEST")
    ...
    parser.add_argument("-m", "--mode",
        choices=["deep", "identity", "snmp", "vuln", "banner", "fuzz",
                 ...
                 "hw", "hpp", "adb", "adb-frida", "adb-dump", "sniff"],
    ...
    parser.add_argument("--quiet", action="store_true",
        help="[v3.9] Tryb cichy — brak ANSI/ASCII-art, tylko wyniki")
    parser.add_argument("--config",
        help="[v3.9] Ścieżka do pliku config.yaml (domyślnie: auto-wykrycie)")
    # sniff-specific
    parser.add_argument("--iface",
        help="[v3.9] Interfejs sieciowy dla sniff (domyślnie: auto)")
    parser.add_argument("--duration", type=int, default=60,
        help="[v3.9] Czas sniff/adb-dump w sekundach (domyślnie: 60)")
    parser.add_argument("--bpf",
        help="[v3.9] Filtr BPF dla sniff (np. 'port 443')")
    ...
    # [v3.9] Załaduj config.yaml
    _load_config(args.config)

    if not QUIET:
        print(BANNER)

    # [v3.9] Fix: dynamiczna nazwa skryptu zamiast hardcoded
    ...
    if os.geteuid() != 0:
        ...
        log("warn", f"Uruchom: sudo python3 {_script_name} — dla pełnej funkcjonalności.")
    else:
    ...
    if args.target and args.mode:
        ...
        elif args.mode == "adb-dump":  adb_audit(dump_only=True, dump_duration=args.duration)
    ...
    # Jeśli podano tylko --mode sniff bez --target
    if args.mode == "sniff":
        sniff_scan(iface=args.iface, duration=args.duration, bpf_filter=args.bpf or "")
        sys.exit(0)
    ...
    while True:
        ...
        if   cmd_type == "quit":     save_cache(CACHE); log("info", "Żegnaj."); sys.exit(0)
            ...
            sniff_scan(iface=arg.get("iface"), duration=arg.get("duration",60), bpf_filter=arg.get("bpf",""))
