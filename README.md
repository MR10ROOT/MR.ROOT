...
Zaawansowane narzędzie do rekonesansu, analizy sieci lokalnej i fuzzingu aplikacji webowych, zaprojektowane z myślą o pracy na Kali NetHunter (Android).
Napisanew Pythonie, oparte na nmap, NSE, SearchSploit oraz autorskich, wielowątkowych modułach HTTP.
...
all-net Full Scan sieci – ping sweep + automatyczny pełny skan każdego wykrytego hosta
h Pomoc
q / exit Wyjście
...
· Mobile Scan – dedukuje typ urządzenia poprzez skanowanie specyficznych portów:
    Android ADB (5555), Apple Lockdown (62078), KDE Connect (1714–1764), AirDroid (8888), AirPlay (7000). Dodatkowo analizuje rekordy ZeroConf/mDNS pod kątem usług Apple i Google Cast.
...
· Network Scan (all-net) – „bomba dywanowa”: wykonuje ping sweep, pyta o zgodę, a następnie uruchamia Full Scan dla każdego hosta w sieci.
    Na koniec generuje zbiorczy plik JSON ze statystykami.
...
# 4. Zainstaluj nmap i exploitdb w systemie
...
# Skanowanie podatności na wszystkich portach (wolne – tylko przy pełnej świadomości obciążenia)
sudo python3 recon3.py -t 192.168.1.1 -m vuln --full-ports

# Tryb z weryfikacją certyfikatów SSL (niepentesterski)
...
└── html/
    └── banner_192-168-101-1_20260404_172536.html # raport HTML z sekcją nagłówków bezpieczeństwa
```
...
· Tryb Full Scan (all / -m all) – wszystkie moduły sekwencyjnie (v3.1)
· Zbieranie statystyk do końcowego raportu JSON (v3.1)
· Integracja z SearchSploit (Exploit-DB) dla wykrytych usług (v3.2)
· Analizator nagłówków HTTP (HSTS, CSP, Info-leak) (v3.2)
· Omijanie ograniczeń routingu NetHunter chroot (v3.3)
· Oznaczanie urządzeń mobilnych (📱/💻) w ping sweep (v3.4)
· Moduły Mobile Scan (m) i Mobile Fuzzer (mf) (v3.4)
· Masowy audyt all-net z możliwością pominięcia hosta przez Ctrl+C (v3.4)
