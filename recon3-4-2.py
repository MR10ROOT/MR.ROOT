#!/usr/bin/env python3
"""
MR.ROOT Scanner v3.4 — NetHunter Edition
...
{C.GREEN}{C.BOLD}
  ...
  ██║ ╚═╝ ██║██║  ██║    ██║  ██║╚██████╔╝╚██████╔╝   ██║
  ╚═╝     ╚═╝╚═╝  ╚═╝    ╚═╝  ╚═╝ ╚═════╝  ╚═════╝    ╚═╝{C.RESET}
{C.DIM}        Mobilny Skaner Sieci | by MR.ROOT | NetHunter Edition v3.4{C.RESET}
"""
...
{c(C.CYAN+C.BOLD, "═══ KOMENDY INTERAKTYWNE ═══")}
  ...
  {c(C.GREEN+C.BOLD, "m <IP>")}           {c(C.YELLOW+C.BOLD, "Mobile Scan — porty ADB/iOS/AirPlay/KDE Connect + mDNS fingerprint")}
...
def ping_sweep(network=None):
    ...
    for host in sorted(nm.all_hosts(), key=sort_key):
        if nm[host].state() == "up":
            ...
            # [v3.4] Wykrywanie urządzeń mobilnych na podstawie vendora MAC i hostname
...
def mobile_scan(ip):
    """
    Skanuje dedykowane porty urządzeń mobilnych i wykonuje fingerprinting
    przez mDNS (Bonjour/ZeroConf). Wykrywa ADB, iOS Lockdown, AirPlay,
    KDE Connect, AirDroid i inne usługi mobilne.
    """
        ...
        for port, desc in sorted(MOBILE_PORTS.items()):
            if port in tcp:
                ...
                if state == "open":
                    ...
                    log("ok", f"  {c(C.GREEN+C.BOLD, f'OPEN')} TCP/{port:<6} {c(C.YELLOW, desc):<40} {c(C.DIM, detail)}")
                    res["open_ports"].append({
                        "port": port, "desc": desc, "service": service,
                        "product": product, "version": version
                    })
                    # Szybki hint na podstawie otwartego portu
    ...
    # --- [2] mDNS fingerprinting pod kątem iOS/Android ---
    ...
    try:
        ...
        if ip in nm2.all_hosts():
            ...
            if raw_mdns:
                ...
                # Wypisz surowe rekordy mDNS (skrócone)
                print()
...
def identity_scan(ip):
    ...
    # [v3.4] Słowa kluczowe mDNS wskazujące na platformę
    ...
    for proto, port, script, name in [
        ...
        try:
            ...
            if ip in nm.all_hosts():
                ...
                # [v3.4] Parsowanie mDNS pod kątem iOS/Android
...
# =========================
# [v3.4] FULL SCAN SIECI — sweep + full_scan dla każdego hosta
# =========================
def full_scan_network(network=None, wordlist_file=None):
    """
    Komenda: all-net [CIDR]
    1. Ping sweep podanej sieci (lub wykrytej automatycznie)
    2. Wyświetla listę znalezionych hostów i pyta o potwierdzenie
    3. Odpala full_scan() dla każdego hosta sekwencyjnie
    4. Zapisuje zbiorczy raport master JSON
    Ctrl+C między hostami = pomiń do następnego
    Ctrl+C × 2 szybko = przerwij całość
    """
    master_div = c(C.GREEN+C.BOLD, "█" * 60)
    print(f"\n{master_div}")
    ...
    # --- Krok 2: Full Scan każdego hosta ---
    ts_start  = datetime.datetime.now()
    all_reports = []
    total     = len(alive)
    ...
    for idx, host in enumerate(alive, 1):
        ...
        try:
            ...
            report["_sweep_info"] = host   # dołącz dane z sweep (mac, vendor, hostname)
            all_reports.append(report)
        except KeyboardInterrupt:
            print()
            log("warn", f"Host {ip} — POMINIĘTY (Ctrl+C). Kontynuuję od następnego...")
            all_reports.append({"ip": ip, "skipped": True, "_sweep_info": host})
            time.sleep(0.4)   # okno na Ctrl+C × 2 żeby wyjść całkowicie

    # --- Zbiorczy raport master ---
    ts_end   = datetime.datetime.now()
...
def parse_interactive_command(raw):
    ...
    # "all-net [CIDR]" — Full Scan całej sieci
    if lower == "all-net":
    ...
    # Pozostałe komendy z jednym argumentem IP
...
def main():
    ...
    parser = argparse.ArgumentParser(description="MR.ROOT Scanner v3.4")
    parser.add_argument("-t", "--target",
        help="Cel skanowania (IP, IPv6, CIDR lub lista np. 192.168.1.1,192.168.1.2)")
    parser.add_argument("-m", "--mode",
        choices=["deep", "identity", "snmp", "vuln", "banner", "fuzz", "sweep", "mobile", "all", "all-net"],
    ...
    while True:
        ...
        if   cmd_type == "quit":     save_cache(CACHE); log("info", "Żegnaj."); sys.exit(0)
            ...
            # Generujemy tymczasowy słownik mobilny (katalogi typowe dla Androida/iOS)
            ...
            try:
                os.remove(tmp_wl)   # Sprzątamy po sobie
            except OSError:
                pass
